...
vocab_file <- ifelse(is.null(args$vocab), "NULL", args$vocab)
...
bootstrap_analysis <- function(tokens, vocab, n_bootstrap_samples = 100) {
    ...
    for (i in 1:n_bootstrap_samples) {
        bootstrap_tokens <- sample(names(counts), size = length(tokens), replace = TRUE, prob = counts)
        bootstrap_counts <- table(bootstrap_tokens)

        # Add 0 counts for vocab words not in bootstrap_tokens
        if (vocab_file != "NULL") {
            missing_words <- setdiff(vocab, names(bootstrap_counts))
            bootstrap_counts[missing_words] <- 0
        }

        if (max_tokens != "NULL" && max_tokens < length(bootstrap_counts)) {
            stratum <- ifelse(bootstrap_counts > median_counts, "high", "low")
...
if (vocab_file != "NULL") {
    ...
    missing_words <- setdiff(vocab, names(counts))
    counts[missing_words] <- 0
}
print(paste('New vocab size:', length(counts)))
