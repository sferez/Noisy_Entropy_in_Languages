...
:description: Apply NLP Offensive detection to a Twitter CSV file.
...
def detect_offensive(texts, batch_size=BATCH_SIZE):
    """
    Detects the offensive of a list of texts using the TweetNLP model and returns the corresponding label-to-id.
    ...
    >>> detect_offensive(['I am so happy', 'You are so ugly'])
    >>> [0, 1]
    """
    offensive = []
...
def process_file(fp):
    """
    Processes a CSV file and adds the offensive column.
...
def process_file_chunk(fp, num_lines):
    """
    Processes a CSV file in chunks and adds the offensive column.
...
if __name__ == "__main__":
    """
    Command Line Interface of the offensive detection script.
